...
buildscript {
    ...
    dependencies {
        classpath 'io.skippy:skippy-gradle:0.0.9'
    }
}
...
buildscript {
    ...
    dependencies {
        classpath 'io.skippy:skippy-gradle:0.0.10-SNAPSHOT'
    }
}
...
`skippyAnalyze` triggers a Skippy Analysis that captures
...
`skippy:analyze` triggers a Skippy Analysis that captures
- a hash for each class file in the project and
- coverage data for each skippified test.
...
Skippy's JUnit libraries are used to skippify tests. Skippy will make skip-or-execute decisions for all skippified tests
based on the data in the `skippy` folder that was generated by Skippy's [build plugins](#build-plugins).
...
dependencies {
    testImplementation 'io.skippy:skippy-junit4:0.0.9'
}
```
...
dependencies {
    testImplementation 'io.skippy:skippy-junit4:0.0.10-SNAPSHOT'
}
```
...
The Skippy class rule intelligently decides whether to execute or skip tests based on the data in the `skippy` folder 
that was generated by Skippy's [build plugins](#build-plugins).
...
dependencies {
    testImplementation 'io.skippy:skippy-junit5:0.0.10-SNAPSHOT'
}
```
...
The Skippy extension intelligently decides whether to execute or skip tests based on the data in the `skippy` folder
that was generated by Skippy's [build plugins](#build-plugins)
